...
export default function TenantLayout() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
      <header style={{ background: 'var(--accent2)', color: 'var(--accent-fg)', padding: '0 1.5rem', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        <nav style={{ width: 200, flexShrink: 0, background: 'var(--surface)', borderRight: '1px solid var(--border)', padding: '1.25rem 0.75rem', display: 'flex', flexDirection: 'column', gap: 2 }}>
          {[
            { to: '/tenant',         label: 'My Unit',   icon: '🏠', end: true },
            { to: '/tenant/history', label: 'History',   icon: '📋' },
          ].map(item => (
